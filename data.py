...
def convert_to_hf_dataset(data_indexed, inp_pad_length=23, out_pad_length=65, quiet=False):
    ...
    # 1 is index of SOS
    attention_mask = [[1] * len(ex.x_indexed) + [0] * (inp_pad_length - len(ex.x_indexed)) for ex in data_indexed] #useful for batching- says which positions in the matrix are actually used. Diff questions are diff lengths
    encodings = {
        'input_ids': inputs,
