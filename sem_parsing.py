...
#part 2 operates over geo-query set.
# inputs and outputs
#
def _parse_args():
    """
    ...
    parser.add_argument('--decode_type', type=str, default='BASIC', help='what type of decoding to use (BASIC, ORACLE, or FANCY)')
...
if __name__ == '__main__':
    ...
    train_data_indexed, dev_data_indexed, test_data_indexed, indexer = index_datasets_one_indexer(train, dev, test, args.decoder_len_limit)

    # Print some examples from the dataset
    print("%i train exs, %i dev exs, %i input types, %i output types" % (len(train_data_indexed), len(dev_data_indexed), len(indexer), len(indexer)))
    print("Max input length: " + repr(max([len(ex.x_indexed) for ex in train_data_indexed + dev_data_indexed + test_data_indexed])))
    if not args.quiet:
    ...
    if args.eval_from_checkpoint:
        ...
        decode_basic(model, indexer, train_dataset, num_exs=10) # just to make sure no crashes
        train_seq2seq_model(model, train_dataset, dev_dataset, args)
        model = model.to("cpu")  # if training on the GPU, moves to the CPU for evaluation
